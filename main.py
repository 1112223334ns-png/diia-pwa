...
PWA_URL = "https://54ea4241-8157-43fb-9ca8-198f7d0dd46b-00-3d0k52f2s1oxa.janeway.replit.dev/"  # Твоя посилання
...
async def choose_subscription(callback: CallbackQuery, state: FSMContext):
    ...
    if sub_type == "sub_test":
        ...
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute("UPDATE users SET code=?, subscription_type='test', expiry_time=?, active=1 WHERE user_id=?", (new_code, expiry, user_id))
        conn.commit()
        conn.close()
...
async def approve_crypto(callback: CallbackQuery):
    ...
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET code=?, subscription_type='paid', active=1, expiry_time=NULL WHERE user_id=?", (new_code, user_id))
    conn.commit()
    conn.close()
